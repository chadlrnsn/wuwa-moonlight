#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: GA_Common_AirAttack

#include "Basic.hpp"

#include "Engine_structs.hpp"
#include "GA_Base_classes.hpp"


namespace SDK
{

// BlueprintGeneratedClass GA_Common_AirAttack.GA_Common_AirAttack_C
// 0x0010 (0x0540 - 0x0530)
class UGA_Common_AirAttack_C final : public UGA_Base_C
{
public:
	struct FPointerToUberGraphFrame               UberGraphFrame_GA_Common_AirAttack_C;              // 0x0530(0x0008)(ZeroConstructor, Transient, DuplicateTransient)
	bool                                          _0__;                                              // 0x0538(0x0001)(Edit, BlueprintVisible, ZeroConstructor, DisableEditOnInstance, IsPlainOldData, NoDestructor)
	uint8                                         Pad_3842[0x3];                                     // 0x0539(0x0003)(Fixing Size After Last Property [ Dumper-7 ])
	float                                         BlockTime;                                         // 0x053C(0x0004)(Edit, BlueprintVisible, ZeroConstructor, DisableEditOnInstance, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

public:
	void ExecuteUbergraph_GA_Common_AirAttack(int32 EntryPoint);
	void K2_ActivateAbility();
	void K2_OnEndAbility(bool bWasCancelled);
	void OnCompleted_5D118C384AE61F1C80292E8133A9FB93();
	void OnBlendOut_5D118C384AE61F1C80292E8133A9FB93();
	void OnInterrupted_5D118C384AE61F1C80292E8133A9FB93();
	void OnCancelled_5D118C384AE61F1C80292E8133A9FB93();
	void OnTick_5D118C384AE61F1C80292E8133A9FB93();
	void OnCompleted_5D118C384AE61F1C80292E810CE84E38();
	void OnBlendOut_5D118C384AE61F1C80292E810CE84E38();
	void OnInterrupted_5D118C384AE61F1C80292E810CE84E38();
	void OnCancelled_5D118C384AE61F1C80292E810CE84E38();
	void OnTick_5D118C384AE61F1C80292E810CE84E38();
	void OnCompleted_5D118C384AE61F1C80292E8132E47F3E();
	void OnBlendOut_5D118C384AE61F1C80292E8132E47F3E();
	void OnInterrupted_5D118C384AE61F1C80292E8132E47F3E();
	void OnCancelled_5D118C384AE61F1C80292E8132E47F3E();
	void OnTick_5D118C384AE61F1C80292E8132E47F3E();

public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"GA_Common_AirAttack_C">();
	}
	static class UGA_Common_AirAttack_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<UGA_Common_AirAttack_C>();
	}
};
static_assert(alignof(UGA_Common_AirAttack_C) == 0x000008, "Wrong alignment on UGA_Common_AirAttack_C");
static_assert(sizeof(UGA_Common_AirAttack_C) == 0x000540, "Wrong size on UGA_Common_AirAttack_C");
static_assert(offsetof(UGA_Common_AirAttack_C, UberGraphFrame_GA_Common_AirAttack_C) == 0x000530, "Member 'UGA_Common_AirAttack_C::UberGraphFrame_GA_Common_AirAttack_C' has a wrong offset!");
static_assert(offsetof(UGA_Common_AirAttack_C, _0__) == 0x000538, "Member 'UGA_Common_AirAttack_C::_0__' has a wrong offset!");
static_assert(offsetof(UGA_Common_AirAttack_C, BlockTime) == 0x00053C, "Member 'UGA_Common_AirAttack_C::BlockTime' has a wrong offset!");

}

